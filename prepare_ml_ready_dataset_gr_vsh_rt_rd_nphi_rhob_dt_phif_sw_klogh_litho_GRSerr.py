...
# ==================== CONFIGURATION ====================
# Window size for GR Serration calculation (in meters)
# This controls the vertical window for calculating GR standard deviation
# - Smaller windows (1-2m): High resolution, captures fine heterogeneity
# - Medium windows (3-5m): Balanced resolution, typical reservoir scale
# - Larger windows (6-10m): Captures broader heterogeneity patterns
GR_SERRATION_WINDOW_SIZE = 3.0  # meters (default: 3.0)

# Input and output file paths
INPUT_FILE = 'Dataset_logs_core_v4_cleaned.csv'
OUTPUT_FILE = 'ML_ready_GR_VSH_RT_RD_NPHI_RHOB_DT_PHIF_SW_KLOGH_GRSerr_facies_litho.csv'
# ========================================================

# Load the dataset
print("="*60)
print("GR SERRATION DATA PREPARATION")
print("="*60)
print(f"Configuration:")
print(f"  - Input file: {INPUT_FILE}")
print(f"  - Output file: {OUTPUT_FILE}")
print(f"  - GR Serration window: {GR_SERRATION_WINDOW_SIZE} meters")
print("="*60 + "\n")
...
# Note: GR_Serration will be added after calculation
...
# NEW STEP: Calculate GR Serration (rolling std of GR) using configured window size
print(f"\nStep 8a: Calculating GR Serration (rolling std) in {GR_SERRATION_WINDOW_SIZE}m window...")
...
for well in wells:
    ...
    # Calculate GR serration using rolling window standard deviation
    depths = well_data['Depth'].values
    gr_values = well_data['GR'].values
    gr_serrations = np.zeros(len(depths))
    
    for i in range(len(depths)):
        # Find points within the window centered at current depth
        current_depth = depths[i]
        window_mask = np.abs(depths - current_depth) <= GR_SERRATION_WINDOW_SIZE / 2
        
        # Need at least 2 points to calculate standard deviation
        if np.sum(window_mask) >= 2:
            window_gr = gr_values[window_mask]
            # Calculate standard deviation
            gr_serrations[i] = np.std(window_gr, ddof=1)  # ddof=1 for sample std
        else:
            # If not enough points, use 0 (no serration)
            gr_serrations[i] = 0
    
    # Assign serrations back to the dataframe
    df_imputed.loc[well_indices, 'GR_Serration'] = gr_serrations

# Add GR_Serration to feature logs
feature_logs.append('GR_Serration')

# Check for any NaN values in GR_Serration and handle them
gr_serration_nans = df_imputed['GR_Serration'].isna().sum()
if gr_serration_nans > 0:
    print(f"Warning: {gr_serration_nans} NaN values in GR_Serration. Filling with 0...")
    df_imputed['GR_Serration'].fillna(0, inplace=True)

print(f"GR_Serration calculated (window={GR_SERRATION_WINDOW_SIZE}m). Range: [{df_imputed['GR_Serration'].min():.4f}, {df_imputed['GR_Serration'].max():.4f}]")
print(f"GR_Serration mean: {df_imputed['GR_Serration'].mean():.4f}, std: {df_imputed['GR_Serration'].std():.4f}")

# Print some examples to show the feature values
print("\nExample GR_Serration values by facies:")
for facies in sorted(df_imputed['Equinor facies'].unique()):
    facies_data = df_imputed[df_imputed['Equinor facies'] == facies]['GR_Serration']
...
# Save the ML-ready dataset (includes facies label, one-hot encoded lithology, and GR_Serration)
df_final.to_csv(OUTPUT_FILE, index=False)
print(f"\nML-ready dataset saved as: {OUTPUT_FILE}")
if GR_SERRATION_WINDOW_SIZE != 3.0:
    print(f"Note: Using non-default window size of {GR_SERRATION_WINDOW_SIZE}m (default is 3.0m)")
