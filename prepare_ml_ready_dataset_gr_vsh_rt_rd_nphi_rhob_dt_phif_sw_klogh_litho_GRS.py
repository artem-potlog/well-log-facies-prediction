...
# ==================== CONFIGURATION ====================
# Default window size for GR Slope calculation (in meters)
DEFAULT_GR_SLOPE_WINDOW = 0.1  # meters

# Parse command line arguments
parser = argparse.ArgumentParser(description='Prepare ML-ready dataset with GR Slope engineered feature')
parser.add_argument('--gr-slope-window', type=float, default=DEFAULT_GR_SLOPE_WINDOW,
                    help=f'Window size for GR Slope calculation in meters (default: {DEFAULT_GR_SLOPE_WINDOW})')
...
window_size_gr_slope = args.gr_slope_window
input_file = args.input

# Generate output filename based on window size if not specified
if args.output is None:
    if window_size_gr_slope == DEFAULT_GR_SLOPE_WINDOW:
        output_filename = 'ML_ready_GR_VSH_RT_RD_NPHI_RHOB_DT_PHIF_SW_KLOGH_GRS_facies_litho.csv'
    else:
        # Include window size in filename if not default
        window_str = str(window_size_gr_slope).replace('.', 'p')
        output_filename = f'ML_ready_GR_VSH_RT_RD_NPHI_RHOB_DT_PHIF_SW_KLOGH_GRS{window_str}m_facies_litho.csv'
else:
...
print(f"GR Slope window size: {window_size_gr_slope} meters")
print("="*60 + "\n")
...
# Note: GR_Slope will be added after calculation
...
# NEW STEP: Calculate GR Slope (d(GR)/dz) using configured window size
print(f"\nStep 8a: Calculating GR Slope (d(GR)/dz) in {window_size_gr_slope} m window...")
...
for well in wells:
    ...
    # Calculate GR slope using centered differences where possible
    ...
    for i in range(len(depths)):
        ...
        window_mask = np.abs(depths - current_depth) <= window_size_gr_slope / 2
        ...
        if np.sum(window_mask) >= 2:
            ...
            if len(window_depths) > 1:
                ...
                gr_slopes[i] = coeffs[0]  # Slope is the first coefficient
            else:
                gr_slopes[i] = 0
        else:
            # If not enough points, use simple forward/backward difference
...
# Add GR_Slope to feature logs
feature_logs.append('GR_Slope')

# Check for any NaN values in GR_Slope and handle them
gr_slope_nans = df_imputed['GR_Slope'].isna().sum()
if gr_slope_nans > 0:
    print(f"Warning: {gr_slope_nans} NaN values in GR_Slope. Filling with 0...")
    df_imputed['GR_Slope'].fillna(0, inplace=True)

print(f"GR_Slope calculated (window={window_size_gr_slope}m). Range: [{df_imputed['GR_Slope'].min():.4f}, {df_imputed['GR_Slope'].max():.4f}]")
print(f"GR_Slope mean: {df_imputed['GR_Slope'].mean():.4f}, std: {df_imputed['GR_Slope'].std():.4f}")
...
# Save the ML-ready dataset (includes facies label, one-hot encoded lithology, and GR_Slope)
