...
# Define the actual column names in the dataset (PHIF and SW removed)
actual_columns = {
    'Well': 'Well Name',
    'Depth': 'MD, m',
...
# Define the feature logs (labels excluded from this list, PHIF and SW removed)
# Note: Engineered features will be added after calculation
feature_logs = ['GR', 'VSH', 'RT', 'RD', 'NPHI', 'RHOB', 'DT', 'KLOGH']
...
# Step 3: Realistic value checks for logs (KLOGH excluded, PHIF and SW removed)
...
# 4. Opposition Index (correlation between GR and NPHI in window)
# MODIFIED: Using GR-NPHI correlation instead of GR-PHIF since PHIF is removed
print("\nCalculating Opposition Index (GR-NPHI correlation in window)...")
...
for well in wells:
    ...
    depths = well_data['Depth'].values
    gr_values = well_data['GR'].values
    nphi_values = well_data['NPHI'].values  # Using NPHI instead of PHIF
    ...
    for i in range(len(depths)):
        ...
        if np.sum(window_mask) >= 3:  # Need at least 3 points for meaningful correlation
            window_gr = gr_values[window_mask]
            window_nphi = nphi_values[window_mask]  # Using NPHI instead of PHIF
            
            if np.std(window_gr) > 0 and np.std(window_nphi) > 0:
                corr, _ = pearsonr(window_gr, window_nphi)
...
# Note about OppositionIndex modification
print("\nNOTE: OppositionIndex modified to use GR-NPHI correlation instead of GR-PHIF (PHIF removed)")
...
# Save the ML-ready dataset with all engineered features (PHIF and SW excluded)
output_filename = 'ML_ready_GR_VSH_RT_RD_NPHI_RHOB_DT_KLOGH_AllEngineered_facies_litho.csv'
df_final.to_csv(output_filename, index=False)
print(f"\nML-ready dataset with all engineered features (no PHIF/SW) saved as: {output_filename}")
...
print(f"\n=== FEATURE SUMMARY ===")
print(f"Total features: {len(feature_logs)}")
print(f"Base logs (8): {[f for f in feature_logs if f in ['GR', 'VSH', 'RT', 'RD', 'NPHI', 'RHOB', 'DT', 'KLOGH']]}")
print(f"Engineered features (7): {engineered_features}")
print(f"REMOVED: PHIF, SW")
print(f"MODIFIED: OppositionIndex now uses GR-NPHI correlation instead of GR-PHIF")
