...
# ==================== CONFIGURATION ====================
# Default window size for Opposition Index calculation (in meters)
DEFAULT_OPPOSITION_INDEX_WINDOW = 0.5  # meters

# Parse command line arguments
parser = argparse.ArgumentParser(description='Prepare ML-ready dataset with Opposition Index engineered feature')
parser.add_argument('--opposition-index-window', type=float, default=DEFAULT_OPPOSITION_INDEX_WINDOW,
                    help=f'Window size for Opposition Index calculation in meters (default: {DEFAULT_OPPOSITION_INDEX_WINDOW})')
...
window_size_opposition_index = args.opposition_index_window
input_file = args.input

# Generate output filename based on window size if not specified
if args.output is None:
    if window_size_opposition_index == DEFAULT_OPPOSITION_INDEX_WINDOW:
        output_filename = 'ML_ready_GR_VSH_RT_RD_NPHI_RHOB_DT_PHIF_SW_KLOGH_OppositionIndex_facies_litho.csv'
    else:
        # Include window size in filename if not default
        window_str = str(window_size_opposition_index).replace('.', 'p')
        output_filename = f'ML_ready_GR_VSH_RT_RD_NPHI_RHOB_DT_PHIF_SW_KLOGH_OppositionIndex{window_str}m_facies_litho.csv'
else:
...
print(f"Opposition Index window size: {window_size_opposition_index} meters")
print("="*60 + "\n")
...
# Note: OppositionIndex will be added after calculation
...
# NEW STEP: Calculate Opposition Index (GR-PHIF correlation in window) using configured window size
print(f"\nStep 8a: Calculating Opposition Index (GR-PHIF correlation) in {window_size_opposition_index} m window...")
...
for well in wells:
    ...
    for i in range(len(depths)):
        ...
        window_mask = np.abs(depths - current_depth) <= window_size_opposition_index / 2
        ...
        if np.sum(window_mask) >= 3:
            ...
            # Calculate correlation if both variables have non-zero variance
    ...
    # Assign indices back to the dataframe
    df_imputed.loc[well_indices, 'OppositionIndex'] = opposition_indices

# Add OppositionIndex to feature logs
feature_logs.append('OppositionIndex')

# Check for any NaN values in OppositionIndex and handle them
opposition_index_nans = df_imputed['OppositionIndex'].isna().sum()
if opposition_index_nans > 0:
    print(f"Warning: {opposition_index_nans} NaN values in OppositionIndex. Filling with 0...")
    df_imputed['OppositionIndex'].fillna(0, inplace=True)

print(f"OppositionIndex calculated (window={window_size_opposition_index}m). Range: [{df_imputed['OppositionIndex'].min():.4f}, {df_imputed['OppositionIndex'].max():.4f}]")
print(f"OppositionIndex mean: {df_imputed['OppositionIndex'].mean():.4f}, std: {df_imputed['OppositionIndex'].std():.4f}")
...
# Save the ML-ready dataset (includes facies label, one-hot encoded lithology, and OppositionIndex)
