"""
Random Forest Classification - Version 6: Multi-Scenario without ROC-AUC

This is V5 without ROC-AUC optimization (which performed poorly).
...
3. Two optimization metrics (custom_sand and f1_weighted only)

Total: 19 × 2 = 38 models per full run

Improvements over V5:
- Removed ROC-AUC optimization (60% worse performance)
- Faster training (38 vs 57 models)
- Cleaner ensemble (only reliable models)
- Better accuracy expected

Author: Based on V5, optimized for production
Date: 2026-01-05
"""
...
class V6MultiScenarioClassifier:
    """
    Multi-Scenario Facies Classification WITHOUT ROC-AUC
    
    Trains 38 models (19 feature combinations × 2 metrics) using GroupKFold CV
    Includes integrated visualizations and comprehensive analysis
    
    Only uses reliable metrics:
    - custom_sand: Petroleum engineering focus
    - f1_weighted: Balanced performance
    """
    
    def __init__(self, dataset_path, grid_type='standard', output_dir='v6_output'):
        """
        Initialize V6 classifier
    ...
    def load_data(self):
        ...
        print("\n" + "="*80)
        print("V6 MULTI-SCENARIO FACIES CLASSIFICATION")
        print("GroupKFold CV - Custom Sand & F1 Weighted Only (No ROC-AUC)")
        print("="*80)
    ...
    def train_all_scenarios(self, feature_combos=None, metrics=None, n_jobs=-1):
        """
        Train all scenarios with custom_sand and f1_weighted only
        
        Parameters:
        -----------
        feature_combos : list or None
            List of feature combination names. If None, use all 19.
        ...
        # V6: Only use custom_sand and f1_weighted (NO ROC-AUC)
        if metrics is None:
            metrics = ['custom_sand', 'f1_weighted']
        
        # Filter out roc_auc_ovr if accidentally included
        metrics = [m for m in metrics if m != 'roc_auc_ovr']
        ...
        print("\n" + "="*80)
        print("V6 TRAINING CONFIGURATION")
        print("="*80)
        print(f"Feature combinations: {len(feature_combos)}")
        print(f"Optimization metrics: {len(metrics)} (custom_sand, f1_weighted)")
    ...
    def visualize_well_predictions(self, target_well, predictions_df, all_probs=None):
        """
        Create comprehensive visualizations for a well
    ...
    def _create_facies_log(self, df, well_name, well_name_safe, viz_dir):
        ...
        ax.set_title('V6 Ensemble Facies Proportions\n(No ROC-AUC)')
        ax.invert_yaxis()
        ax.set_xlim([0, 1])
        ...
        ax.set_title('V6 Ensemble Uncertainty')
        ax.invert_yaxis()
        ax.set_xlim([0, 1])
        ax.legend()
        ax.grid(True, alpha=0.3)
        ...
        ax.set_xlim([0, 1])
        ax.set_title('True vs V6 Ensemble')
        ax.invert_yaxis()
        
        plt.suptitle(f'V6 Facies Proportion Log - {well_name}',
    ...
    def _create_comprehensive_analysis(self, df, well_name, well_name_safe, viz_dir):
        ...
        plt.suptitle(f'V6 Comprehensive Analysis - {well_name}',
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
    ...
    def create_summary_visualizations(self):
        ...
        plt.suptitle('V6 Multi-Scenario Summary (No ROC-AUC)', fontsize=16, fontweight='bold')
        plt.tight_layout()
        
        output_path = viz_dir / 'v6_summary_analysis.png'
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"  ✓ Saved: {output_path.name}")
        plt.close()
...
def main():
    ...
    print("\n" + "="*80)
    print("V6 MULTI-SCENARIO FACIES CLASSIFICATION - MAIN SCRIPT")
    print("Custom Sand & F1-Weighted Only (No ROC-AUC)")
    print("="*80)
    ...
    print("  1. Baseline Test (FC_01-03) - 6 models, ~5 min")
    print("  2. Lithology Path (FC_01-06) - 12 models, ~15 min - RECOMMENDED")
    print("  3. Fluid Isolation (FC_01-03,10-12) - 12 models, ~15 min")
    print("  4. Full Progressive (FC_01-18) - 36 models, ~1.5 hours")
    print("  5. Test Forward Selection Winner (FC_19) - 2 models, ~3 min")
    ...
    if mode == '1':
        ...
        output_dir = 'v6_fluid_isolation'
        
    elif mode == '4':
        # Full progressive test
        grid_type = 'standard'
        feature_combos = None  # All 18
        output_dir = 'v6_full_progressive'
        
    elif mode == '5':
        # Forward selection winner only
        grid_type = 'standard'
        feature_combos = ['FC_19_forward_optimal']
        output_dir = 'v6_forward_winner'
    ...
    classifier = V6MultiScenarioClassifier(
    ...
    # Train all scenarios (V6: only custom_sand and f1_weighted)
    classifier.train_all_scenarios(
        feature_combos=feature_combos,
        metrics=['custom_sand', 'f1_weighted'],  # NO ROC-AUC!
        n_jobs=-1
    )
    ...
    print("\n" + "="*80)
    print("V6 COMPLETE! All results and visualizations generated.")
    print("="*80)
    ...
    print("  - v6_summary_analysis.png - Overall model comparison")
