"""
Random Forest Classification with Uncertainty Quantification - Version 3
COMPLETE VERSION: Combines all features from V1 and V2
- V2's enhanced model combination visualization and reporting
- V1's detailed individual well facies proportion logs
- Best of both worlds for comprehensive ensemble analysis
"""
...
def load_data():
    ...
    print("="*80)
    print("RANDOM FOREST ENSEMBLE - UNCERTAINTY QUANTIFICATION V3")
    print("Complete version with all visualizations from V1 and V2")
    print("="*80)
...
def visualize_model_combinations(train_combinations, test_wells, available_train_wells):
    """Create a visualization showing which wells are used in each model (FROM V2)"""
...
def select_wells_for_ensemble(df):
    """Select wells for ensemble training and testing - Enhanced version (FROM V2)"""
...
def train_ensemble_models(df, train_combinations, test_wells, features):
    """Train multiple RF models on different well combinations - Enhanced reporting (FROM V2)"""
    ...
    # Summary statistics (V2 enhancement)
...
def create_enhanced_visualizations(proportions_df, all_facies, model_accuracies, model_info, test_wells, combo_fig):
    """Create enhanced visualizations with model combination information (FROM V2)"""
    
    # Create output directory
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = f"RF_Uncertainty_Ensemble_V3_{timestamp}"
...
def create_facies_proportion_logs(proportions_df, all_facies, output_dir):
    """Create detailed facies proportion logs for each test well (FROM V1)"""
    ...
    for well in test_wells:
        ...
        print(f"Creating facies log for well {well_name}...")
        ...
        legend_elements = []
        for facies in np.unique(np.concatenate([true_facies, ensemble_pred])):
            facies_name = FACIES_NAMES.get(facies, f'F{facies}')
            legend_elements.append(plt.Rectangle((0,0),1,1, fc=facies_colors.get(facies, 'gray'), 
                                               alpha=0.7, label=facies_name))
        ax.legend(handles=legend_elements, bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=6)
        
        plt.suptitle(f'Facies Proportion Log - Well {well_name}', fontsize=14, fontweight='bold')
        plt.tight_layout()
        
        # Save individual well plot
        well_filename = os.path.join(output_dir, f'Well_{well_name}_facies_log.png')
        plt.savefig(well_filename, dpi=300, bbox_inches='tight')
        plt.show()
        
        print(f"✓ Saved: {well_filename}")
    
    print(f"\n✓ Created individual facies logs for {len(test_wells)} test wells")
...
def save_detailed_results(proportions_df, model_info, output_dir):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("ENSEMBLE UNCERTAINTY QUANTIFICATION REPORT - V3\n")
        f.write("="*60 + "\n\n")
...
def main():
    ...
    # Select wells and show combinations (V2 enhanced)
    ...
    # Train ensemble (V2 enhanced reporting)
    ...
    print("\n" + "="*60)
    print("PROCESS COMPLETE - V3 WITH ALL FEATURES")
    print("="*60)
    print(f"✓ Trained {len(model_accuracies)} models successfully")
    print(f"✓ Created enhanced ensemble analysis (V2 features)")
    print(f"✓ Created individual well facies logs (V1 features)")
    print(f"✓ Generated model combination visualizations")
    print(f"✓ All results saved to: {output_dir}/")
    print("="*60)
