...
# ==================== CONFIGURATION ====================
# Default window size for NTG Slope calculation (in meters)
DEFAULT_NTG_SLOPE_WINDOW = 1.0  # meters
# Default VSH threshold for NTG calculation
DEFAULT_VSH_THRESHOLD = 0.35  # fraction

# Parse command line arguments
parser = argparse.ArgumentParser(description='Prepare ML-ready dataset with NTG Slope engineered feature')
parser.add_argument('--ntg-slope-window', type=float, default=DEFAULT_NTG_SLOPE_WINDOW,
                    help=f'Window size for NTG Slope calculation in meters (default: {DEFAULT_NTG_SLOPE_WINDOW})')
parser.add_argument('--vsh-threshold', type=float, default=DEFAULT_VSH_THRESHOLD,
                    help=f'VSH threshold for NTG calculation (default: {DEFAULT_VSH_THRESHOLD})')
...
window_size_ntg_slope = args.ntg_slope_window
vsh_threshold = args.vsh_threshold
input_file = args.input

# Generate output filename based on parameters if not specified
if args.output is None:
    if window_size_ntg_slope == DEFAULT_NTG_SLOPE_WINDOW and vsh_threshold == DEFAULT_VSH_THRESHOLD:
        output_filename = 'ML_ready_GR_VSH_RT_RD_NPHI_RHOB_DT_PHIF_SW_KLOGH_NTGSlope_facies_litho.csv'
    else:
        # Include parameters in filename if not default
        window_str = str(window_size_ntg_slope).replace('.', 'p')
        vsh_str = str(vsh_threshold).replace('.', 'p')
        output_filename = f'ML_ready_GR_VSH_RT_RD_NPHI_RHOB_DT_PHIF_SW_KLOGH_NTGSlope{window_str}m_VSH{vsh_str}_facies_litho.csv'
else:
...
print(f"NTG Slope window size: {window_size_ntg_slope} meters")
print(f"VSH threshold for NTG: {vsh_threshold}")
print("="*60 + "\n")
...
# Note: NTG_Slope will be added after calculation
...
# NEW STEP: Calculate NTG Slope (Net-to-Gross slope across window halves) using configured parameters
print(f"\nStep 8a: Calculating NTG Slope (Net-to-Gross slope) in {window_size_ntg_slope} m window with VSH threshold {vsh_threshold}...")
...
for well in wells:
    ...
    for i in range(len(depths)):
        ...
        window_mask = np.abs(depths - current_depth) <= window_size_ntg_slope / 2
        
        # Need at least 4 points for two meaningful halves
        if np.sum(window_mask) >= 4:
            ...
            # Calculate NTG for each half (NTG = fraction where VSH < threshold)
            ntg_top = np.mean(window_vsh[:mid_idx] < vsh_threshold)
            ntg_bottom = np.mean(window_vsh[mid_idx:] < vsh_threshold)
            
            # Slope is top minus bottom (positive = improving upward, i.e., higher NTG upward)
...
# Add NTG_Slope to feature logs
feature_logs.append('NTG_Slope')

# Check for any NaN values in NTG_Slope and handle them
ntg_slope_nans = df_imputed['NTG_Slope'].isna().sum()
if ntg_slope_nans > 0:
    print(f"Warning: {ntg_slope_nans} NaN values in NTG_Slope. Filling with 0...")
    df_imputed['NTG_Slope'].fillna(0, inplace=True)

print(f"NTG_Slope calculated (window={window_size_ntg_slope}m, VSH_thresh={vsh_threshold}). Range: [{df_imputed['NTG_Slope'].min():.4f}, {df_imputed['NTG_Slope'].max():.4f}]")
print(f"NTG_Slope mean: {df_imputed['NTG_Slope'].mean():.4f}, std: {df_imputed['NTG_Slope'].std():.4f}")
...
# Save the ML-ready dataset (includes facies label, one-hot encoded lithology, and NTG_Slope)
