...
# Define the feature logs (label excluded from this list)
...
# Step 1: Drop rows with empty values in selected feature logs (label may be NaN at this step)
...
# Step 6: Apply log10 transformation to RT and RD before normalization
print("\nStep 6: Applying log10 transformation to RT and RD...")
df_imputed['RT'] = np.log10(df_imputed['RT'])
df_imputed['RD'] = np.log10(df_imputed['RD'])

# Step 7: Global normalization using StandardScaler (features only)
print("\nStep 7: Applying global normalization to selected features...")
...
print(f"After dropping unlabeled facies: {stats['after_drop_unlabeled']}")
...
# Save the ML-ready dataset (Well and Depth kept; includes Equinor facies label)
output_filename = 'ML_ready_GR, VSH, RT, RD, NPHI, RHOB, DT, PHIF, SW, KLOGH, facies.csv'
