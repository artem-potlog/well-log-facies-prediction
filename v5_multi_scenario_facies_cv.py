"""
Random Forest Classification - Version 5: Multi-Scenario Cross-Validation Approach

This implements the professor's full multi-scenario approach:
1. Different feature combinations (19 options: 18 manual + 1 forward selection)
2. Different train-validation splits (GroupKFold CV with well-based groups)
3. Different optimization metrics (3 options)

Total: 19 × 3 = 57 models per full run

Key improvements over V4:
- GroupKFold cross-validation (no data leakage)
- Pipeline with proper preprocessing
- Hyperparameter tuning per scenario
- 54-model ensemble with rich diversity
- Progressive feature testing strategy
- Integrated visualizations and analysis
- All-in-one comprehensive script

Author: Based on V5_IMPLEMENTATION_PLAN.md
...
from feature_combinations import FEATURE_COMBINATIONS, get_feature_list
...
class V5MultiScenarioClassifier:
    """
    Multi-Scenario Facies Classification with Cross-Validation
    
    Trains up to 57 models (19 feature combinations × 3 metrics) using GroupKFold CV
    Includes integrated visualizations and comprehensive analysis
    
    Feature combinations include:
    - 18 manual progressive combinations (domain knowledge)
    - 1 forward selection optimal (data-driven discovery)
    """
    
    def __init__(self, dataset_path, grid_type='standard', output_dir='v5_output'):
        """
        Initialize V5 classifier
    ...
    def load_data(self):
        ...
        print("\n" + "="*80)
        print("V5 MULTI-SCENARIO FACIES CLASSIFICATION")
        print("GroupKFold Cross-Validation with Feature & Metric Diversity")
        print("="*80)
    ...
    def configure_wells(self, train_wells=None, test_wells=None, interactive=True):
        ...
        Parameters:
        -----------
        train_wells : list or None
            Wells for training. If None, use interactive selection or defaults.
        test_wells : list or None
            Wells for testing. If None, use interactive selection or defaults.
        interactive : bool
            If True, allow user to select wells interactively
        """
        # Get available wells from data
        ...
        if interactive and train_wells is None:
            ...
            print("These wells will NOT be used in training")
            ...
            if train_wells is not None:
                ...
                # Try to use first 4 wells as default
                self.train_wells = available_wells[:min(4, len(available_wells)-1)]
            
            if test_wells is not None:
                self.test_wells = test_wells
            else:
                # Use remaining wells for testing
                self.test_wells = [w for w in available_wells if w not in self.train_wells]
        
        # Validate configuration
        if len(self.train_wells) < 4:
            print(f"\n⚠ WARNING: Only {len(self.train_wells)} training wells selected.")
            print(f"   GroupKFold with k=4 requires at least 4 wells.")
            print(f"   Adjusting n_splits to {len(self.train_wells)}")
        
        # Display configuration
        print("\n" + "-"*80)
        print("WELL CONFIGURATION")
        print("-"*80)
        print(f"Training wells (GroupKFold CV with k={min(4, len(self.train_wells))}):")
    ...
    def train_all_scenarios(self, feature_combos=None, metrics=None, n_jobs=-1):
        """
        Train all 30 scenarios (10 features × 3 metrics)
        
        Parameters:
        -----------
        feature_combos : list or None
            List of feature combination names. If None, use all 10.
        metrics : list or None
            List of metric names. If None, use all 3.
        n_jobs : int
        ...
        total_scenarios = len(feature_combos) * len(metrics)
        
        # Prepare training data
        df_train = self.df[self.df['Well'].isin(self.train_wells)].copy()
        groups_train = df_train['Well'].values
        
        # Determine number of CV folds based on number of training wells
        ...
        for fc_name in feature_combos:
            ...
            for metric_name in metrics:
                ...
                print("\n" + "="*80)
                print(f"SCENARIO {scenario_num}/{total_scenarios}: {fc_name} + {metric_name}")
                print("="*80)
                ...
                # Create pipeline (CRITICAL: all preprocessing inside!)
                pipeline = Pipeline([
                    ...
                    ('classifier', RandomForestClassifier(
                        ...
                        n_jobs=1  # Parallel at GridSearchCV level
                    ))
                ])
                ...
                # Train
                print(f"\nTraining with GroupKFold (k={n_splits})...")
                ...
                scenario_id = f"{fc_name}_{metric_name}"
    ...
    def _create_ranking_table(self):
        ...
        print("TOP 10 MODELS")
        print("="*80)
        print(ranking_df[['rank', 'scenario_id', 'cv_mean_score', 'cv_std_score', 'n_features']].head(10).to_string(index=False))
        print("="*80)
    ...
    def predict_ensemble(self, target_well, voting_strategy='equal', top_n=None):
        ...
        -----------
        target_well : str
            Well name (e.g., '15_9-F-11 B')
        voting_strategy : str
            'equal': Equal weight for all models
            'weighted': Weight by CV score
        top_n : int or None
            If not None, use only top N models
        
        Returns:
        --------
        results_df : pd.DataFrame
            Predictions with uncertainty metrics
        """
        print("\n" + "="*80)
        ...
        depth = df_target['DEPTH_MD'].values if 'DEPTH_MD' in df_target.columns else np.arange(len(df_target))
        ...
        # Stack predictions
        all_probs = np.stack(all_probs, axis=0)  # Shape: (n_models, n_samples, 9)
        ...
        # Add probability columns (handle variable number of classes)
        n_classes = avg_probs.shape[1]
        for i in range(n_classes):
            results_df[f'Prob_Class_{i}'] = avg_probs[:, i]
        
        # Add zero probabilities for missing classes (if any)
    ...
    def visualize_well_predictions(self, target_well, predictions_df, all_probs=None):
        """
        Create comprehensive visualizations for a well (V4-style)
        
        Generates:
        - 3-panel facies proportion log
    ...
    def _create_facies_log(self, df, well_name, well_name_safe, viz_dir):
        ...
        ax.set_title('V5 Ensemble Facies Proportions')
        ax.invert_yaxis()
        ax.set_xlim([0, 1])
        ...
        ax.set_title('V5 Ensemble Uncertainty')
        ax.invert_yaxis()
        ax.set_xlim([0, 1])
        ax.legend()
        ax.grid(True, alpha=0.3)
        ...
        ax.set_xlim([0, 1])
        ax.set_title('True vs V5 Ensemble')
        ax.invert_yaxis()
        
        plt.suptitle(f'V5 Facies Proportion Log - {well_name}',
    ...
    def _create_comprehensive_analysis(self, df, well_name, well_name_safe, viz_dir):
        ...
        # Panel 4: True vs Predicted strips
        ax = axes[1, 0]
        for i in range(len(depth)-1):
            pred_color = colors[pred_facies[i]] if pred_facies[i] < 9 else 'gray'
        ...
        plt.suptitle(f'V5 Comprehensive Analysis - {well_name}',
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
    ...
    def create_summary_visualizations(self):
        ...
        # Summary figure with 6 panels
        fig = plt.figure(figsize=(20, 12))
        
        # Panel 1: Model ranking
        ax1 = plt.subplot(2, 3, 1)
        y_pos = np.arange(len(ranking_valid))
        ax1.barh(y_pos, ranking_valid['cv_mean_score'],
                xerr=ranking_valid['cv_std_score'], alpha=0.7)
        ax1.set_yticks(y_pos)
        ax1.set_yticklabels(ranking_valid['scenario_id'], fontsize=8)
        ax1.set_xlabel('CV Mean Score')
        ax1.set_title('Model Ranking')
        ax1.grid(True, alpha=0.3, axis='x')
        ax1.invert_yaxis()
        
        # Panel 2: Score distribution by metric
        ax2 = plt.subplot(2, 3, 2)
        metrics = ranking_valid['metric'].unique()
        for metric in metrics:
            metric_scores = ranking_valid[ranking_valid['metric'] == metric]['cv_mean_score']
            ax2.hist(metric_scores, alpha=0.6, label=metric, bins=10)
        ...
        # Panel 4: Feature combination performance
        ax4 = plt.subplot(2, 3, 4)
        fc_stats = ranking_valid.groupby('feature_combination')['cv_mean_score'].agg(['mean', 'std'])
        fc_stats = fc_stats.sort_values('mean', ascending=False).head(10)
        y_pos = np.arange(len(fc_stats))
        ax4.barh(y_pos, fc_stats['mean'], xerr=fc_stats['std'], alpha=0.7)
        ax4.set_yticks(y_pos)
        ax4.set_yticklabels(fc_stats.index, fontsize=8)
        ax4.set_xlabel('Mean CV Score')
        ax4.set_title('Top 10 Feature Combinations')
        ...
        summary = "V5 MULTI-SCENARIO SUMMARY\n" + "="*35 + "\n\n"
        ...
        for idx, row in ranking_valid.head(3).iterrows():
            summary += f"  {row['rank']}. {row['scenario_id']}\n"
        ...
        plt.suptitle('V5 Multi-Scenario Summary', fontsize=16, fontweight='bold')
        plt.tight_layout()
        
        output_path = viz_dir / 'v5_summary_analysis.png'
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"  ✓ Saved: {output_path.name}")
        plt.close()
...
def main():
    ...
    print("\n" + "="*80)
    print("V5 MULTI-SCENARIO FACIES CLASSIFICATION - MAIN SCRIPT")
    print("="*80)
    ...
    print("  1. Baseline Test (FC_01-03) - 9 models, ~10 min")
    print("  2. Lithology Path (FC_01-06) - 18 models, ~25 min - RECOMMENDED")
    print("  3. Fluid Isolation (FC_01-03,10-12) - 18 models, ~25 min")
    print("  4. Full Progressive (FC_01-19) - 57 models, ~2 hours")
    print("  5. Test Forward Selection Winner (FC_19) - 3 models, ~5 min")
    ...
    if mode == '1':
        # Baseline test
        grid_type = 'standard'
        feature_combos = ['FC_01_rock_only', 'FC_02_rock_clean', 'FC_03_add_perm']
        metrics = ['f1_weighted', 'custom_sand', 'roc_auc_ovr']
        output_dir = 'v5_baseline_test'
        
    elif mode == '2':
        # Lithology path (most important!)
        grid_type = 'standard'
        feature_combos = ['FC_01_rock_only', 'FC_02_rock_clean', 'FC_03_add_perm',
                          'FC_04_fc3_lith', 'FC_05_fc3_lith_relpos', 'FC_06_fc3_lith_relpos_grtex']
        metrics = ['f1_weighted', 'custom_sand', 'roc_auc_ovr']
        output_dir = 'v5_lithology_path'
        
    elif mode == '3':
        # Fluid isolation test
        grid_type = 'standard'
        feature_combos = ['FC_01_rock_only', 'FC_02_rock_clean', 'FC_03_add_perm',
                          'FC_10_fc3_rt', 'FC_11_fc3_rt_rd', 'FC_12_fc3_rt_rd_sw']
        metrics = ['f1_weighted', 'custom_sand', 'roc_auc_ovr']
        output_dir = 'v5_fluid_isolation'
        
    elif mode == '4':
        # Full progressive test
        grid_type = 'standard'
        feature_combos = None  # All 19
        metrics = None  # All 3
        output_dir = 'v5_full_progressive'
        
    elif mode == '5':
        # Forward selection winner only
        grid_type = 'standard'
        feature_combos = ['FC_19_forward_optimal']
        metrics = ['f1_weighted', 'custom_sand', 'roc_auc_ovr']
        output_dir = 'v5_forward_winner'
        
    elif mode == '6':
        # Custom selection
        print("\nAvailable feature combinations:")
        for i, fc_name in enumerate(FEATURE_COMBINATIONS.keys(), 1):
            n_feat = len(FEATURE_COMBINATIONS[fc_name]['features'])
            print(f"  {i:2d}. {fc_name} ({n_feat} features)")
        
        print("\nEnter comma-separated numbers (e.g., 1,2,3):")
        selection = input("Feature combos: ").strip()
        indices = [int(x.strip())-1 for x in selection.split(',') if x.strip().isdigit()]
        fc_list = list(FEATURE_COMBINATIONS.keys())
        feature_combos = [fc_list[i] for i in indices if 0 <= i < len(fc_list)]
        
        print("\nSelect metrics:")
        print("  1. f1_weighted only")
        print("  2. custom_sand only")
        print("  3. Both (f1 + custom_sand)")
        print("  4. All three (f1 + custom_sand + roc_auc_ovr)")
        metric_choice = input("Metrics: ").strip()
        ...
        if metric_choice == '1':
            ...
            metrics = ['f1_weighted', 'custom_sand']
        else:
            metrics = ['f1_weighted', 'custom_sand', 'roc_auc_ovr']
        
        grid_type = 'standard'
        output_dir = 'v5_custom'
        
    else:
        # Default: Forward selection winner
        print("Invalid mode. Using Forward Selection Winner (fastest).")
        grid_type = 'standard'
        feature_combos = ['FC_19_forward_optimal']
        metrics = ['f1_weighted', 'custom_sand', 'roc_auc_ovr']
        output_dir = 'v5_forward_winner'
    ...
    classifier = V5MultiScenarioClassifier(
    ...
    # Configure wells (interactive selection)
    ...
    for test_well in classifier.test_wells:
        try:
            ...
            # Generate visualizations for this well
    ...
    print("\n" + "="*80)
    print("V5 COMPLETE! All results and visualizations generated.")
    print("="*80)
    print(f"\nResults saved to: {output_dir}/")
    print("\nGenerated files:")
    print("  - results/model_ranking.csv - All models ranked")
    ...
    print("  - v5_summary_analysis.png - Overall model comparison")
